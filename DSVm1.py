...
class Problem_Genetic(object):
    ...
    def mutation(self, chromosome, prob):
        ...
        def swap_randomM1(chromosome_aux):
        ...
        def replace_two_randomM2(chromosome_aux):
            ...
            idx = range(len(chromosome))
            i1, i2 = random.sample(idx, 2)
            repl = [chromosome[i1], chromosome[i2]]
            temp = Diff(self.genes, repl)
            chromosome[i1], chromosome[i2] = random.sample(temp, 2)
            return chromosome
        # this is trash
        def random_replace_mutation_one(chromosome_aux):
            chromosome = chromosome_aux
            i1 = random.choice(range(len(chromosome)))
            temp = chromosome[i1]
            new_list = [el for el in self.genes if el != temp]
            chromosome[i1] = random.sample(new_list, 1)
            return chromosome
...
DSVproblem = DSVm1(10,15)
DSVproblem.run(10,False)
