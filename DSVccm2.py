...
class Problem_Genetic(object):
    ...
    def mutation(self, chromosome, prob, cc, tc):
        ...
        def replace_two_randomM2(chromosome_aux):
            ...
            subtract = chromosome + cc
            temp = Diff(self.genes,subtract)
        ...
        for _ in range(len(chromosome)):
            if random.random() < prob:
                aux = replace_two_randomM2(chromosome)
        return aux
...
class DSVccm2:
    ...
    def run(self, k, printRate):
        ...
        def first_part_GA(k):
            cont = 1
            print(
                "--------------------------------------------------------- Genetic Algorithm with a Country Constraint M2 --------------------------------------------------------- \n")
...
DSVproblem = DSVccm2(20,30,4,4)
DSVproblem.run(3,False)
